---
name: quant-trader
description: Quantitative trading via Interactive Brokers TWS — market data, technical analysis, multi-factor scoring, sentiment, fundamentals, portfolio management, and order execution with Plan→Confirm safety model.
installer:
  type: uvx
  package: quant-trader
metadata:
  categories: [finance, trading, mcp]
  tags: [interactive-brokers, quantitative-trading, stock-analysis]
---

# quant-trader

Quantitative trading MCP Server for Claude Code. Connects to Interactive Brokers TWS for US stocks.
...
| Daily Workflow | `morning_briefing`, `intraday_snapshot`, `eod_summary`, `add_manual_pick`, `win_rate_history` |
...
### Morning Briefing
```
"Generate today's morning briefing for my watchlist"
→ Calls morning_briefing with configured watchlist
→ Creates ~/quant-trader-reports/YYYY-MM-DD-morning.md
```

### Multi-Factor Stock Analysis
```
"Score NVDA and tell me if it's worth buying"
→ Calls score_stock("NVDA")
→ Returns 0-100 score with ATR-based entry/target/stop
```
...
7. **Audit log** — all order operations logged to `~/quant-trader-reports/audit.json`
